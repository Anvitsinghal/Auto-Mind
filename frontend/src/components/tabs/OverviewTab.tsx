  ...
  return (
    <div className="space-y-4 sm:space-y-6 p-3 sm:p-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6">
        ...
        <div className="lg:col-span-1 glass-panel-elevated p-4 sm:p-8 flex flex-col items-center justify-center animate-scale-in">
          <h2 className="text-base sm:text-lg font-semibold text-foreground mb-4 sm:mb-6">Overall Vehicle Health</h2>
        ...
        {/* Stats Grid */}
        <div className="lg:col-span-2 grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 gap-2 sm:gap-4">
          <StatusCard
            title="Next Service"
            value="12 Days"
          ...
          <StatusCard
            ...
            icon={<Battery className="h-4 w-4 sm:h-5 sm:w-5" />}
            trend="stable"
            trendValue="Optimal"
            status="success"
            delay={400}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
        {/* Recent Predictions */}
        <div className="glass-panel p-4 sm:p-6 animate-fade-in-up" style={{ animationDelay: '400ms' }}>
          <div className="flex items-center justify-between mb-3 sm:mb-4">
            <h3 className="font-semibold text-foreground text-sm sm:text-base">Recent Predictions</h3>
          ...
          <div className="space-y-2 sm:space-y-3">
            {[
              ...
              <div key={i} className="flex items-center gap-2 sm:gap-4 p-2 sm:p-3 rounded-lg bg-secondary/30 hover:bg-secondary/50 transition-colors">
                <div className={`w-2 h-2 rounded-full flex-shrink-0 ${item.status === 'success' ? 'bg-success' : 'bg-warning'}`} />
                <div className="flex-1 min-w-0">
                  <p className="text-xs sm:text-sm font-medium text-foreground truncate">{item.title}</p>
                  <p className="text-[10px] sm:text-xs text-muted-foreground">Confidence: {item.confidence}%</p>
                </div>
                <span className="text-[10px] sm:text-xs text-muted-foreground flex-shrink-0">{item.time}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="glass-panel p-4 sm:p-6 animate-fade-in-up" style={{ animationDelay: '500ms' }}>
          <div className="flex items-center justify-between mb-3 sm:mb-4">
            <h3 className="font-semibold text-foreground text-sm sm:text-base">Maintenance Schedule</h3>
            <button className="text-[10px] sm:text-xs text-primary hover:text-primary/80 transition-colors">View All</button>
          </div>
          <div className="space-y-2 sm:space-y-3">
            {[
              ...
              <div key={i} className="flex items-center gap-2 sm:gap-4 p-2 sm:p-3 rounded-lg bg-secondary/30">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-xs sm:text-sm font-medium text-foreground truncate">{item.title}</p>
                  <p className="text-[10px] sm:text-xs text-muted-foreground">Due in {item.due}</p>
                </div>
                <span className={`px-1.5 sm:px-2 py-0.5 sm:py-1 rounded text-[10px] sm:text-xs font-medium flex-shrink-0 ${
